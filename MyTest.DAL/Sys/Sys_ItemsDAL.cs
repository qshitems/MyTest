...
namespace MyTest.DAL
{
	...
	/// 数据访问类:Sys_ItemsDAL
	/// </summary>
	public partial class Sys_ItemsDAL
	{
		public Sys_ItemsDAL()
		{}
		#region  BasicMethod
		...
		public bool Exists(string F_Id)
		{
			...
			strSql.Append("select count(1) from Sys_Items");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(MyTest.Model.Sys_ItemsModel model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Sys_Items(");
			strSql.Append("F_Id,F_ParentId,F_EnCode,F_FullName,F_IsTree,F_Layers,F_SortCode,F_DeleteMark,F_EnabledMark,F_Description,F_CreatorTime,F_CreatorUserId,F_LastModifyTime,F_LastModifyUserId,F_DeleteTime,F_DeleteUserId)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@F_IsTree", SqlDbType.Bit,1),
			...
			parameters[4].Value = model.F_IsTree;
			parameters[5].Value = model.F_Layers;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(MyTest.Model.Sys_ItemsModel model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Sys_Items set ");
			strSql.Append("F_ParentId=@F_ParentId,");
			strSql.Append("F_EnCode=@F_EnCode,");
			strSql.Append("F_FullName=@F_FullName,");
			strSql.Append("F_IsTree=@F_IsTree,");
			...
			parameters[3].Value = model.F_IsTree;
			parameters[4].Value = model.F_Layers;
		...
		public bool Delete(string F_Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Sys_Items ");
			strSql.Append(" where F_Id=@F_Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@F_Id", SqlDbType.VarChar,50)			};
			parameters[0].Value = F_Id;
		...
		/// 得到一个对象实体
		/// </summary>
		public MyTest.Model.Sys_ItemsModel GetModel(string F_Id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 F_Id,F_ParentId,F_EnCode,F_FullName,F_IsTree,F_Layers,F_SortCode,F_DeleteMark,F_EnabledMark,F_Description,F_CreatorTime,F_CreatorUserId,F_LastModifyTime,F_LastModifyUserId,F_DeleteTime,F_DeleteUserId from Sys_Items ");
			strSql.Append(" where F_Id=@F_Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@F_Id", SqlDbType.VarChar,50)			};
			parameters[0].Value = F_Id;

			MyTest.Model.Sys_ItemsModel model=new MyTest.Model.Sys_ItemsModel();
		...
		/// 得到一个对象实体
		/// </summary>
		public MyTest.Model.Sys_ItemsModel DataRowToModel(DataRow row)
		{
			MyTest.Model.Sys_ItemsModel model=new MyTest.Model.Sys_ItemsModel();
			if (row != null)
			{
				...
				if(row["F_IsTree"]!=null && row["F_IsTree"].ToString()!="")
				{
					if((row["F_IsTree"].ToString()=="1")||(row["F_IsTree"].ToString().ToLower()=="true"))
					{
						model.F_IsTree=true;
					}
					else
					{
						model.F_IsTree=false;
					}
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select F_Id,F_ParentId,F_EnCode,F_FullName,F_IsTree,F_Layers,F_SortCode,F_DeleteMark,F_EnabledMark,F_Description,F_CreatorTime,F_CreatorUserId,F_LastModifyTime,F_LastModifyUserId,F_DeleteTime,F_DeleteUserId ");
			strSql.Append(" FROM Sys_Items ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Sys_Items T ");
